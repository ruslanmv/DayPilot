...
async def open_settings(page, section_title: str) -> None:
    ...
    # Scoped to the settings rail: "Calendar" also names a primary nav item.
    await page.get_by_label("Settings sections").get_by_role(
        "button", name=section_title, exact=True).click()
...
async def desktop_tour(page) -> None:
    ...
    # --- Calendar: its own header, the connection chip, and the meeting-context
    # settings that decide what a brief may read.
    await goto(page, "#/calendar", ".dp-cal")
    await snap(page, "calendar")

    await reset(page)
    await open_settings(page, "Calendar")
    await snap(page, "calendar-settings")
    # The context allow-list and the planning rules sit below the fold.
    try:
        body = page.locator(".dp-settings-panel__body")
        await body.evaluate("el => el.scrollTop = el.scrollHeight")
        await page.wait_for_timeout(600)
        await snap(page, "calendar-settings-context")
    except Exception as exc:  # noqa: BLE001
        print(f"    (calendar settings scroll skipped — {str(exc).splitlines()[0]})")
    await close_overlay(page)
