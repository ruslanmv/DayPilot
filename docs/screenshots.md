...
| Set | Path | Contents |
|---|---|---|
| Product tour | `docs/assets/screenshots/tour/` | Home, command palette, Focus Mode, Approval Center, Planning, Projects, Documents, Email (dark + light), Calendar + its two Settings screens, project wizard, patch review, Matrix Designer, four Settings sections, first-run onboarding, and four phone screens. |
...
| Slack | `docs/assets/screenshots/slack/` | The workspace, a prepared reply, its provenance expanded, the assistant proposing a rewrite, both Settings → Slack screens, and the workspace on a phone. |
...
Desktop **1440×900** (the Slack workspace at **1536×1024**, because four columns
need the width to photograph as four columns), phone **414×896**, both at
`device_scale_factor=2`. GitHub
...
| `scripts/screenshots/seed_calendar.py` | A connected Outlook (and Slack, so the context allow-list shows both states) and the day around a real meeting. |
| `scripts/screenshots/seed_slack.py` | A morning of Slack, ingested through the real classify/draft pipeline. |
...
| `scripts/screenshots/shoot_slack.py` | The Slack workspace, draft, assistant and settings. |
...
would otherwise crowd its three bullets. The Slack seeder runs **last**, so the
projects, tasks and documents it draws context from already exist — a draft
assembled from an empty workspace would photograph as a draft with no sources.

The Slack seeder also proves a product claim rather than illustrating one: it
posts five messages through `slack_workspace.service.ingest`, and the classifier
decides that three of them deserve a draft. The restraint in that screenshot is
